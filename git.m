...
% A thin MATLAB wrapper for Git Bash.
%
%   Short instructions:
%       Use this exactly as you would use the OS command-line verison of Git.
%           - For help with any command type:
%           git <command> --help
%           command eg. git status --help
%           To Launch a command cheat sheet type
%           git -help
%
%   Long instructions are:
%       This is meant to provide some basic commands
%       Git Documentation:
%           https://git-scm.com/doc
%           Copy and Paste link into a browser
%
...
%       
%       This script was originally taken from the Matlab User Community                                                                                             

% Launch Git Bash Cheat Sheet
if strcmp(varargin{1},'-help')
    if size(varargin,2) == 2
        site = ['https://git-scm.com/docs/git' varargin{2}];
        web(site,'-browser')
    else
        winopen github-git-cheat-sheet.pdf
    end
    return
end
...
try
    ...
    [status,res1] = system('git --version');
    ...
    if status
        ...
        if ~(search||searchx86)
            ...
            errordlg('Please Verify Git is installed and located at C:\Program Files\Git\bin\git.exe')
...
%Refresh Matlab Directory to see changes
folder = pwd;
cd ..
cd(folder)
end
